...
$_SESSION['page'] = "ownerLihatLaporan";
$_SESSION['subPage'] = "ownerLihatLaporan";
if (!isset($_SESSION['username']) && $_SESSION['privilege'] <> 'owner') {
?>
  ...
  <body>
    ...
    <div class="container">
      ...
      <div id="Charts" class="border-bottom border-right border-left rounded-bottom shadow" style="border-color: rgb(222, 226, 230);">
        ...
        <div id="morrisChartJmlHargaBeli" style="height: 20rem" class="">
          <canvas id="chartJmlHargaBeli"></canvas>
        </div>
      </div>
        ...
        <div class="row justify-content-between d-flex align-items-center">
          ...
          <div class="col order-0">
            <div id="action-button" class="responsive-text" style="height: 100%">
              <a href="ownerInputPenjualan.php"><button class="btn btn-outline-dark" title="Tambah Data"><i class="fa fa-plus" aria-hidden="true"></i> Tambah Data</button></a>
...
//      }
      ...
      function updateChart(tahun) {
        $.get('php/chartMaker.php?tahun=' + tahun + "&tipe=chartJmlJual", function(data) {
          var result = jQuery.parseJSON(data);
          var masukanGabung = result[0].split("|");
          var masukanJual = masukanGabung[0].split(",");
          var masukanBeli = masukanGabung[1].split(",");
          console.log(result);
          console.log(masukanGabung);
          chartJmlJual.config.options.scales.yAxes[0].ticks.max = result[1] + 10 - (result[1] % 10);
          chartJmlJual.data.datasets[0].data = masukanJual;
          chartJmlJual.data.datasets.push({});
          chartJmlJual.data.datasets[1].label="Jumlah Transaksi Pembelian";
          chartJmlJual.data.datasets[1].backgroundColor=[
                        'rgba(255, 72, 5, 0.2)',
                        'rgba(255, 72, 5, 0.2)',
                        'rgba(255, 72, 5, 0.2)',
                        'rgba(255, 72, 5, 0.2)',
            ...
            chartJmlJual.data.datasets[1].borderWidth=1;
            chartJmlJual.data.datasets[1].data=masukanBeli;
        ...
        //chart jumlah harga beli
        $.get('php/chartMaker.php?tahun=' + tahun + "&tipe=chartJmlHargaBeli", function(data) {
          var result = jQuery.parseJSON(data);
          var masukan = result[0].split(",");
          // debugger;
          for (var i = masukan.length - 1; i >= 0; i--) {
          ...
          var maxPerJuta = result[1] / 1000000;
          chartJmlHargaBeli.config.options.scales.yAxes[0].ticks.max = maxPerJuta + 100 - (maxPerJuta % 100);
          // chartJmlHarga.config.options.scales.yAxes[0].ticks.max = 500;
          chartJmlHargaBeli.data.datasets[0].data = masukan;
          chartJmlHargaBeli.update();
        });
      }
