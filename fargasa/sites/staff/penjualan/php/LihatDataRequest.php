...
if (isset($tipe)) {
    if ($tipe == 'Init') {
        ...
        $keyword = $_GET['query'];
        $query = "SELECT DISTINCT * from penjualan where tipe LIKE '%" . trim($keyword) . "%' OR "
        ...
        while ($row = mysqli_fetch_array($data)) {
            ...
            $output .= '
                      <tr>
                        ...
                        <td>' . $row['warna'] . '</td>
                        <td>' . $row['nopol'] . '</td>
                        <td>' . $conn->intToRupiah((int)$row['hrg_jual'] + (int)$row['fee_mediator'] + (int)$row['pajak'] + (int)$row['rekondisi']) . '</td>
                        ' . $statusBadge . '
                        <td><div id="action-button" class="d-inline" style="width:100%;">
                            ...
                            <a class="btn-action btn btn-primary text-white btn-sm edit" data-href="' . $row['id_penjualan'] . '" title="Edit"><i class="fa fa-pencil" aria-hidden="true"></i></a>
                            <a class="btn-action btn btn-danger text-white btn-sm delete" data-href="' . $row['id_penjualan'] . '" title="Delete"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
